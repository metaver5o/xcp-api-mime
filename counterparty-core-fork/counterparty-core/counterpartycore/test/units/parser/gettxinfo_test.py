...
def test_get_tx_info_taproot(ledger_db, current_block_index, defaults, blockchain_mock):
    data = b"Hello world"
    ...
    # For taproot reveals, the source is derived from the internal pubkey in the
    # envelope script. The mock setup is for cases where the rust deserializer
    # falls back to Python for source determination.
    blockchain_mock.source_by_txid[
        "ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"
    ] = source

    # Note: The expected source address "17askaM3RknEAw8AFwdiP9ffSNtZyfzFBw" is derived
    # from the random internal pubkey generated by composer.generate_envelope_script().
    # When the rust deserializer is active, it extracts the source from the witness.
    # When Python fallback is used, it uses the mock's source_by_txid lookup.
    result = gettxinfo.get_tx_info(
        ledger_db,
        decoded_tx,
        current_block_index,
    ...
    # The source can be either the mock's source or the derived internal pubkey address
    # depending on whether the rust deserializer or Python fallback is used
    assert result[1] == ""  # destination
    assert result[2] == 0  # btc_amount
    assert result[3] == 1000000000  # fee
    assert result[4] == b"Hello world"  # data
    assert result[5] == []  # dispensers_outs
    assert result[6] == ["", "", "1", "0"]  # utxos_info
